...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/23 14:14:24 by mondrew           #+#    #+#             */
/*   Updated: 2020/09/23 22:12:33 by mondrew          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list	*ft_lst_new(char type, float x, float y, float radius, char elem_char)
{
	t_list	*new;
...
void	ft_fill_images(t_data *first, t_list **lst_ptr, char *str)
{
	t_list	*lst;
	int		i; // x
	int		j; // y
...
int		ft_print_images(t_data *first, t_list **lst_ptr)
{
	char	*str;

	if (!(str = malloc(sizeof(char) * \
			((first->width * first->height) + (first->height - 1) + 1))))
		return (-1);
	ft_fill_background(first, str);
	ft_fill_images(first, lst_ptr, str);
	ft_putstr(str);
	ft_putstr("\n");
	free(str);
	return (0);
...
int		mini_paint(char *filename)
{
	...
	if ((ft_print_images(&first, &lst)) == -1)
	{
		fclose(ifp);
		if (lst)
			ft_lst_clear(lst);
